...
- 文本为详细的**英文**放射科发现（实测 208/208 全英文，早期备注误写为中文已修正），例如：
  - `"An irregular-shaped mass in the right inguinal region with enhancement at the edge..."` (长，带鉴别诊断)
  - `"Subpleural inflammation of the left lower lung"` (短)
  - `"High-density nodule in upper lobe of left lung"` (中)
- 每条描述通常包含 3 要素：病灶类型（mass / nodule / cyst ...）+ 解剖位置（left lower lung ...）+ 形态鉴别（irregular / considered as ...）
...
| Exp9 | **VoxTell zero-shot (raw 英文临床描述)** | **0.1327** | — | **13.4%** | 全量 208 cases / 783 masks |
...
```
Text ("irregular mass in the right inguinal region")
       ↓
...
| 模式 | 例子 | 理念 |
|------|------|------|
| **raw** | `"An irregular-shaped mass in the right inguinal region with enhancement at the edge..."` | 原文直送，保留病灶类型+解剖位置+形态三要素 |
...
1. **英文长句原文 > 英文关键词 (2.3×)**：早期单 case 测试（s0000）显示 raw=0 而 structured (`mass`)=0.30，结论反向。全量统计后 raw 远胜 structured，原因推测：
   - Qwen3-Embedding-4B 能从长句中提取病灶**类别 + 解剖部位 + 形态描述**的联合语义
   - 纯关键词丢失了所有空间定位信息（`"right lower lung"`、`"upper pole"` 等方位词）
   - regex 抽取器 fallback=`lesion` 过于宽泛，许多 mask 结构化 prompt 全部退化为 `lesion`
...
3. **Dice 分布双峰**：60% 全零 + 13% ≥0.5，说明 VoxTell 在能理解的 prompt 上表现很好，在另一部分 prompt 上直接失败 — 提示后续可从"哪类 prompt 失败"入手改进（长尾病灶词、含鉴别诊断的长句、罕见解剖位置等）。
